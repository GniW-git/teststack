...
const TokenDecimal = 3;//token decimal
const MaximumStakingAmount = 1000000;//maximum staking amount
...
async function loadInitialData(sClass) {
    try {
        ...
        $('#total-locked-tokens').html(`${totalLockedTokens / 10**TokenDecimal} ${_CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol}`);
        ...
        if(minStakeAmount) {
            minA = `${(minStakeAmount / 10 ** TokenDecimal).toLocaleString()} ${_CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol}`;
        ...
        $(`#${sClass} .max-stake-amount`).html(`${(MaximumStakingAmount).toLocaleString()} ${_CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol}`);
...
async function stackTokens() {
    try {
        ...
        let tokenToTransfer = addDecimal(nTokens, TokenDecimal);
        ...
        balMainUser = Number(balMainUser) / (10 ** TokenDecimal);
...
async function unstackTokens() {
    try {
        ...
        balMainUser = Number(balMainUser.stakeAmount) / (10 ** TokenDecimal);
...
async function refreshBal(sClass) {
    ...
    let nBalMainW = nBalMain / 10**TokenDecimal;
    ...
    let nBalStackW = nBalStack.stakeAmount / 10**TokenDecimal;
    ...
    $('#user-reward-balance').html(`Reward: ${rewardBal / 10 ** TokenDecimal} ${_CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol}`);
}
